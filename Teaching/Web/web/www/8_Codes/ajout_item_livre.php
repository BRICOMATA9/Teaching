...
<?php
    ...
    if (isset($_POST['button_ajout'])) {
        ...
        $description = isset($_POST["decription"])? $_POST["decription"] : "";
        ...
        else if((!empty($nom)) AND (!empty($auteur)) AND (!empty($edition)) AND (!empty($annee_publication)) AND (!empty($description)) AND (!empty($photos)) AND (!empty($prix)) AND (!empty($stock))){

            if ($db_found) {
                
                //recherche parmis les livres si ce nom exite déjà 
                ...
                if (mysqli_num_rows($result) == 0) {
                    ...
                    $sql = "SELECT * FROM vetements WHERE description = '$description'";
                    $result = mysqli_query($db_handle, $sql);
                    //si pas de résultat
                    if (mysqli_num_rows($result) == 0) {
                        ...
                        $sql = "INSERT INTO livres(ID_vendeur, nom,auteur,edition,annee_publication,type,genre,description,prix,photos,photo2,video,stock,nb_vendus,date_ligne) VALUES ('$ID_vendeur','$nom','$auteur','$edition','$annee_publication','$type','$genre','$description','$prix','$photos','$photo2','$video','$stock',0,NOW())";
                        $result = mysqli_query($db_handle, $sql); 

                        $valid = "Votre livre ". $nom ." a été ajouté à la vente !";
...
<head>
    ...
    <title>Ajout Livre | ECE Amazon</title>
    <meta charset="utf-8">
...
<body>
    ...
    <header>
        <div class="container">
            <!-- Titre -->
            <h2 class="titre">Mettre en vente un livre</h2>
                    ...
                    <form method="post">
                        <table>
                            ...
                            <tr>
                                ...
                                <td><input type="number" name="annee_publication"></td>
                            </tr>
                            <tr>
                                <td>Type: </td>
                                <td>
                            ...
                            <tr>
                                <td>Description: </td>
                                <td><textarea name="decription" rows="5"></textarea></td>
                            </tr>
                            <tr>
