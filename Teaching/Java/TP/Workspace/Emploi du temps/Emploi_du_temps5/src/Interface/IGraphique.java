  ...
  private void Addcourstojour(List<List<TP>> tabCours) {
		...
		for(int jours=0;jours<5;jours++){
			...
			for (int i=0;i<=(nbcours*8)-1;i=i+8){
				...
				//la matiere
				SJour[i+2]="  Module : "+ModuleDAO.getInstance().getModuleById(c.getIdModule().getIdModule()).getNom()+"\n";
				StyleJour[i+2]="cours";
				//la salle
				SJour[i+3]="  Salle : "+c.getIdSalle().getIdSalle()+"\n";
				StyleJour[i+3]="salle";
				//le prof
				SJour[i+4]="  Prof : "+EnseignantDAO.getInstance().getEnseignantById(c.getIdEnseignant().getIdPersonne()).getNom()+"\n";
				StyleJour[i+4]="prof";
				//le section
