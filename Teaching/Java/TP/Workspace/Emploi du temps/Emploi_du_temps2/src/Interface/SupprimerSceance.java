...
public class SupprimerSceance extends JFrame implements ActionListener {
    ...
    public SupprimerSceance(Recherche service) throws Exception {
    	...
    	if (source==enregistrer){
			try{
	    		if(type.getSelectedItem().toString().equals("Cours")){
					...
					if (service.supprimerCours(cours)) setVisible(false);
					else JOptionPane.showMessageDialog(this,"La seance n'existe pas","Erreur",JOptionPane.ERROR_MESSAGE);
					...
					if (service.supprimerTP(tp)) setVisible(false);
					else JOptionPane.showMessageDialog(this,"La seance n'existe pas","Erreur",JOptionPane.ERROR_MESSAGE);
				}
			}catch(Exception eh){
