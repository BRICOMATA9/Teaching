    	...
    	if (source==enregistrer){
				try{
					if(supprimer.getSelectedItem() instanceof Enseignant)
						EnseignantDAO.getInstance().supprimerEnseignant((Enseignant)supprimer.getSelectedItem());
					else if(supprimer.getSelectedItem() instanceof Etudiant)
						EtudiantDAO.getInstance().supprimerEtudiant((Etudiant)supprimer.getSelectedItem());
					else if(supprimer.getSelectedItem() instanceof Module)
						ModuleDAO.getInstance().supprimerModule((Module)supprimer.getSelectedItem());
					else if(supprimer.getSelectedItem() instanceof Groupe)
						GroupeDAO.getInstance().supprimerGroupe((Groupe)supprimer.getSelectedItem());
					else if(supprimer.getSelectedItem() instanceof Section)
						SectionDAO.getInstance().supprimerSection((Section)supprimer.getSelectedItem());
					else if(supprimer.getSelectedItem() instanceof TP)
						TPDAO.getInstance().supprimerTP((TP)supprimer.getSelectedItem());
					else if(supprimer.getSelectedItem() instanceof Cours)
						CoursDAO.getInstance().supprimerCours((Cours)supprimer.getSelectedItem());
					else if(supprimer.getSelectedItem() instanceof Salle)
						SalleDAO.getInstance().supprimerSalle((Salle)supprimer.getSelectedItem());
					else if(supprimer.getSelectedItem() instanceof Batiment)
						BatimentDAO.getInstance().supprimerBatiment((Batiment)supprimer.getSelectedItem());
					else if(supprimer.getSelectedItem() instanceof Batiment)
						BatimentDAO.getInstance().supprimerBatiment((Batiment)supprimer.getSelectedItem());
					setVisible(false);
				}catch(Exception eh){
