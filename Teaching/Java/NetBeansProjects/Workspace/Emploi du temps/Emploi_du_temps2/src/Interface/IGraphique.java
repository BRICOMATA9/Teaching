...
public class IGraphique implements ActionListener {
  ...
  private void Addcourstojour(List<List<TP>> tabCours) {
		int nbcours;
		JTextPane textpane;
		...
		for(int jours=0;jours<5;jours++){
			...
			nbcours = listec.size();
			String[] SJour = new String[7*nbcours];
			String[] StyleJour = new String[7*nbcours];
			int j =0;
			for (int i=0;i<=(nbcours*7)-1;i=i+7){
				//on va chercher les cours a afficher pour chaque jour
				TP c = listec.get(j);
				SJour[i]= "Heure : "+formatterTime.format(c.getHeureDebut())+"\nDurree : "+
				formatterTime.format(c.getDuree())+"\n";
				StyleJour[i]="horaire";
				//la matiere
				SJour[i+1]="Module : "+c.getIdModule()+"\n";
				StyleJour[i+1]="cours";
				//la salle
				SJour[i+2]="Salle : "+c.getIdSalle()+"\n";
				StyleJour[i+2]="salle";
				//le prof
				SJour[i+3]="Prof : "+c.getIdEnseignant()+"\n";
				StyleJour[i+3]="prof";
				//le section
				SJour[i+4]="Section : "+c.getIdSection()+"\n";
				StyleJour[i+4]="groupe";
				//le groupe
				SJour[i+5]="Groupe : "+(c.getIdGroupe()==0?"":c.getIdGroupe())+"\n";
	...
	private List<List<TP>> visualiser_EDT(Jours Semaine) throws Exception {
		List<List<TP>> liste_cours = trie_par_jour(service.getAllSeance());
