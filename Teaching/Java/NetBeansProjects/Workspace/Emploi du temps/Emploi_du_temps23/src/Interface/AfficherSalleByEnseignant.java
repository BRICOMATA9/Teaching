...
public class AfficherSalleByEnseignant<T> extends JFrame implements ActionListener {
    ...
    public AfficherSalleByEnseignant() throws Exception{
        ...
        supprimer = new JComboBox<Object>(EnseignantDAO.getInstance().getAll().toArray());
	    ...
	    Stream<TP> listeTP = TPDAO.getInstance().getAllSeance();
	    Stream<Salle> liste = listeTP.filter(a -> a.getIdEnseignant().getIdPersonne().equals(((Enseignant)supprimer.getSelectedItem()).getIdPersonne()))					
	    		.map(a -> a.getIdSalle())
	    		.distinct();
