...
main()
{
 ...
 scanf("%d",&val);
 b=0;
 for (i=0;i<n&&b==0;i++) if (t[i]==val) b=1;
 if (b==1) printf("%d existe\n",val);
 for (i=1;i<n;i++)
  {
 ...
 if (val<t[0]||val>t[n-1]) b=0;
    ...
    { 
     ...
     while (sup>inf&&b==0)
      {
       milieu=(sup+inf)/2;
       if (val>milieu) inf=milieu;
        else if (val<milieu) sup=milieu;
           else b=1;
      }
    }
 if (b==1) printf("\n%d existe\n",val);
