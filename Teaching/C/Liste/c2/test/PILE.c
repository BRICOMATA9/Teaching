...
typedef struct no {int elm;pile svt;}noeud;
pile creer() /****************************************creer*/
{
...
void ghiles(pile *l,int n)
{
 int e;
 if (n==0);
  else {scanf("%d",&e);empiler(l,e);ghiles(l,n-1);}
}
pile lifo()  /******************************************LIFO*/
{
 pile l=NULL;
...
{
 if(!vide(l)||sommet(l)>e) printf("la valeur n'existe pas");
  else
   if(sommet(l)<e) {recherche1(desempiler(l),e);}
...
int recherche2 (pile l,int e) /*****************recherche*/
{
 if(!vide(l)) return(0);
   ...
   if (sommet(l)==e) return(1);
    else {recherche2(desempiler(l),e);}
}
void modifier (pile l,int x,int y) /************************modifier*/
{
 if(!vide(l));
  else 
   if (sommet(l)==x){l=desempiler(l);empiler(&l,y);modifier(desempiler(l),x,y);}
    else {modifier(desempiler(l),x,y);}
} 
void insert(pile l,int e)/************insertion ds une pile trier*/
{
  if(!vide(l)) empiler(&l,e);
   else
    if(sommet(l)<e) {insert(desempiler(l),e);}
     else empiler(&l,e);
}
void suprimmer (pile *l,int e)/************************desempilermer**/
{
 if(!vide(*l));
   ...
   if (sommet(*l)==e) {*l=desempiler(*l);suprimmer(l,e);}
    else {*l=desempiler(*l);suprimmer(l,e);}
}
...
void tripile(pile *tete,pile p)
{
 if(!(*tete)->svt);
   ...
   if(!p) tripile(&(*tete)->svt,(*tete)->svt);
    else 
     if((*tete)->elm>p->elm)
     {
      int q=(*tete)->elm;
