...
typedef struct {
	typelem mot; // les éléments de la pile s
	int occ;
} typelem2;

typedef struct {
	typelem t[max]; // pile p
	int sommet;
} pile;
typedef struct {
	typelem2 t[max]; // pile s
	int sommet;
} pile2;

/// les fonctions de la pile p *******************************
pile initpile() {
	pile p;
	p.sommet = -1;
	return (p);
...
char *sommetpile(pile p) // cette fonction ne peut pas retourner typelem qui est
												 // un tableau de char
{
	return p.t[p.sommet];
}
...
typelem2 sommetpile2(pile2 p) { return p.t[p.sommet]; }
...
pile Const_Pile() {
	int i, n, b;
	pile p = initpile(), r = initpile();
	typelem x, mot;
	char *y =
			(char *)malloc(30 * sizeof(char)); // car sommetpile retourne un char *
	...
	for (i = 1; i <= n; i++) {
		...
		gets(mot);
		b = 1;
		while (!pilevide(p) && b == 1) {
			y = sommetpile(p);
			if (strlen(mot) > strlen(y)) {
				desempiler(&p, x);
				empiler(&r, x);
			} else
				b = 0;
