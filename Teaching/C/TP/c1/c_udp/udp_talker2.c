...
int main() {
	...
	while (strcmp(msg, ".")) {
		if ((msgLength = strlen(msg)) > 0) {
			...
			if (sendto(socketDescriptor, msg, msgLength, 0, servinfo->ai_addr,
		...
		// Invite de commande pour l'utilisateur et lecture des caractères jusqu'à
		// la limite MAX_MSG. Puis suppression du saut de ligne en mémoire tampon.
		// Comme ci-dessus.
		puts("Saisie du message : ");
		memset(msg, 0, sizeof msg); // Mise à zéro du tampon
		scanf(" %" xstr(MAX_MSG) "[^\n]%*c", msg);
