		   	...
		   	if ($pseudoexiste == 0)
		   	{
		   		...
		   		if($mailexiste == 0)
			   	{
			   		...
			   		if ($passw1 == $passw2)
			   		{
			   			...
			   			$reqid = $bdd->query("SELECT * FROM comptes ORDER BY comptes . ID DESC LIMIT 1");
			   			$maxid = $reqid->fetch();
			  			
			   			$updatepanier = $bdd->prepare("UPDATE paniers SET IDAcheteur = ? WHERE IDAcheteur = ?");
			   			$updatepanier->execute(array($maxid['ID'], 0));
			   			$_SESSION['id'] = $maxid['ID'];
				        $_SESSION['pseudo'] = $maxid['Pseudonyme'];
				        $_SESSION['email'] = $maxid['Email'];
				        $_SESSION['type'] = $maxid['TypeCompte'];
...
<div id="section">
	<form id="<?php if ($cree == 1) { echo "hideform";} else { echo "shoform";} ?>" action="" method="post">
		...
		<table>
			...
			<tr>
				<td>Pseudonyme : </td>   
				<td><input type="text" name="pseudo" placeholder = "Pseudonyme" value="<?php echo "$pseudo" ?>" /></td>
			...
			<td>Pays : </td>   
				<td><input type="text" name="pays" placeholder = "Pays" value="<?php echo "$Pays" ?>" /></td>
			...
			<tr>   
				<td>Mot de Passe : </td>  
				<td><input type="password" name="passw1" placeholder="Mot de passe" /></td> 
				<td>
					<?php 
			...
			<tr>   
				...
				<td><input type="password" name="passw2" placeholder="Mot de passe" /></td> 
				<td>
					<?php 
