  ...
  private void Addcourstojour(List<List<TP>> tabCours) throws Exception {
		...
		for(int jours=0;jours<5;jours++){
			...
			for (int i=0;i<=(nbcours*8)-1;i=i+8){
				...
				TP c = listec.get(j);
				//type
				SJour[i]="  Type : "+(c.getIdGroupe().getIdGroupe()!=null?"TP":"Cours")+"\n";
				StyleJour[i]="cours";
				//Heure
				SJour[i+1]= "  De : "+formatterTime.format(c.getHeureDebut())+"\n  A : "+formatterTime.format(c.getDuree())+"\n";
				StyleJour[i+1]="horaire";
				//la matiere
				SJour[i+3]="  Module : "+ModuleDAO.getInstance().getById(c.getIdModule().getIdModule()).getNom()+"\n";
				StyleJour[i+3]="cours";
				//la salle
				SJour[i+2]="  Salle : "+c.getIdSalle().getIdSalle()+(c.getSalleBis().getIdSalle()==null?"":" & "+c.getSalleBis().getIdSalle())+"\n";
				StyleJour[i+2]="salle";
				//le prof
				...
				//le groupe
				SJour[i+6]=(c.getIdGroupe().getIdGroupe()==null?"":"  Groupe : "+c.getIdGroupe().getIdGroupe())+"\n";
	...
	public void addtolisteCours(Jours Semaine) {
		try {
			...
			JOptionPane.showMessageDialog(null,eh.getMessage(),"Erreur",JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private List<List<TP>> visualiser_EDT(Jours Semaine) throws Exception {
		List<List<TP>> liste_cours = trie_par_jour(TPDAO.getInstance().getAllSeance().collect(toList()));
