...
public class EnseignantDAO implements IDAO<Enseignant>{
	...
	public Stream<Enseignant> getAll() throws Exception{
	...
	public <V> Enseignant getById(V idPersonne) throws Exception{
	...
	public Stream<String> getNom(){
		...
		try{
			...
			ResultSet resultProfs = db.Get_DB(t);
			while (resultProfs.next())
				listProfs.add(resultProfs.getString(1));
		}catch(Exception x){
			JOptionPane.showMessageDialog(null ,x.getMessage(),"Erreur",JOptionPane.ERROR_MESSAGE);
		}
		return listProfs.stream();
	...
	public Boolean ajouter(Enseignant enseignant) throws Exception {
