...
import DAO.TPDAO;
import IAfficher.Afficher;
import IAjouter.AjouterBatiment;
import IAjouter.AjouterCours;
import IAjouter.AjouterEnseignant;
import IAjouter.AjouterEtudiant;
import IAjouter.AjouterGroupe;
import IAjouter.AjouterModule;
import IAjouter.AjouterSalle;
import IAjouter.AjouterSection;
import IAjouter.AjouterTP;
import IModifier.ModifierEnseignant;
import IModifier.ModifierEtudiant;
import IModifier.ModifierGroupe;
import IModifier.ModifierModule;
import IModifier.ModifierSalle;
import IModifier.ModifierSeance;
import IModifier.ModifierSection;
import ISupprimer.Supprimer;
import SQL.DB;
...
/*			else if (e.getSource()==button3)
			...
			if (e.getSource()==enseignant0)
				new Afficher<Enseignant>("Enseignant",EnseignantDAO.getInstance().getAllEnseignant());
			else if (e.getSource()==etudiant0)
					new Afficher<Etudiant>("Etudiant",EtudiantDAO.getInstance().getAllEtudiant());
			else if (e.getSource()==groupe0)
				new Afficher<Groupe>("Groupe",GroupeDAO.getInstance().getAllGroupe());
			else if (e.getSource()==matiere0)
				new Afficher<Module>("Module",ModuleDAO.getInstance().getAllModule());
			else if (e.getSource()==promotion0)
				new Afficher<Section>("Section",SectionDAO.getInstance().getAllSection());
			else if (e.getSource()==seance0)
				new Afficher<Cours>("Cours",CoursDAO.getInstance().getAllCours());
			else if (e.getSource()==tp0)
				new Afficher<TP>("TP",TPDAO.getInstance().getAllTP());
			else if (e.getSource()==batiment0)
				new Afficher<Batiment>("Batiment",BatimentDAO.getInstance().getAllBatiment());
			else if (e.getSource()==salle0)
				new Afficher<Salle>("Batiment",SalleDAO.getInstance().getAllSalle());
			...
			if (e.getSource()==enseignant1)
				...
				new Supprimer<Enseignant> (() -> EnseignantDAO.getInstance().getAllEnseignant());
			else if (e.getSource()==etudiant2)
				new Supprimer<Etudiant> (() -> EtudiantDAO.getInstance().getAllEtudiant());
			else if (e.getSource()==groupe2)
				new Supprimer<Groupe> (() -> GroupeDAO.getInstance().getAllGroupe());
			else if (e.getSource()==matiere2)
				new Supprimer<Module> (() -> ModuleDAO.getInstance().getAllModule());
			else if (e.getSource()==promotion2)
				new Supprimer<Section> (() -> SectionDAO.getInstance().getAllSection());
			else if (e.getSource()==salle2)
				new Supprimer<Salle> (() -> SalleDAO.getInstance().getAllSalle());
			else if (e.getSource()==batiment2)
				new Supprimer<Batiment> (() -> BatimentDAO.getInstance().getAllBatiment());
			else if (e.getSource()==seance2)
				new Supprimer<Cours>(() -> CoursDAO.getInstance().getAllCours());
			else if (e.getSource()==tp2)
				new Supprimer<TP>(() -> TPDAO.getInstance().getAllTP());
				...
				new ModifierSeance<Cours>(() -> CoursDAO.getInstance().getAllCours());
			else if (e.getSource()==tp3)
				new ModifierSeance<TP>(() -> TPDAO.getInstance().getAllTP());
