  ...
  private void Addcourstojour(List<List<TP>> tabCours) {
		...
		for(int jours=0;jours<5;jours++){
			...
			for (int i=0;i<=(nbcours*8)-1;i=i+8){
				...
				TP c = listec.get(j);
				//type
				SJour[i]="  Type : "+(c.getIdGroupe()!=0?"TP":"Cours")+"\n";
				StyleJour[i]="cours";
				//Heure
				...
				//la matiere
				SJour[i+2]="  Module : "+service.getModuleById(c.getIdModule()).getNom()+"\n";
				StyleJour[i+2]="cours";
				//la salle
				SJour[i+3]="  Salle : "+c.getIdSalle()+"\n";
				StyleJour[i+3]="salle";
				//le prof
				SJour[i+4]="  Prof : "+service.getEnseignantById(c.getIdEnseignant()).getNom()+"\n";
				StyleJour[i+4]="prof";
				//le section
				SJour[i+5]="  Section : "+c.getIdSection()+"\n";
				StyleJour[i+5]="groupe";
				//le groupe
				SJour[i+6]=(c.getIdGroupe()==0?"":"  Groupe : "+c.getIdGroupe())+"\n";
