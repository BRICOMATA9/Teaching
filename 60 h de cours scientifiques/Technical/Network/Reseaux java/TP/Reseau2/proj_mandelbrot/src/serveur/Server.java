...
public class Server {
    ...
    public Server( int webport ) {
        try {
            ...
            serverC = new ComputingServer( 9090 );
    ...
    public void writeWebResponse( SelectionKey client ) throws Exception {
        SocketChannel socket = (SocketChannel) client.channel();
        String request = (String) client.attachment();
        String fileName = extractFileName( request );
        System.out.println( fileName );
        File fileRequested = new File( fileName );
        if ( fileName.equals( "mandelbrotset.png" ) ) {
            File f = new File( "mandel.txt" );
            // envoi de la requete au serveur de calcul
            ...
            // appel de la methode de gereration de la fractale
